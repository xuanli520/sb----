...
public class AuthorController implements UserResolver {
    ...
    public AuthorController(NovelStore store,CoverUploadService coverUploadService,BookPresentationService bookPresentations,MediaAssetService mediaAssets){this.store=store;this.coverUploadService=coverUploadService;this.bookPresentations=bookPresentations;this.mediaAssets=mediaAssets;}
    @GetMapping("/books")
    ...
    @PutMapping("/books/{bookId}") ApiResponse<BookPresentation> updateBook(HttpServletRequest request,@PathVariable long bookId,@Valid @RequestBody BookUpdateRequest body){CurrentUser u=current(request);u.require(Role.AUTHOR);return ApiResponse.ok(bookPresentations.present(store.updateBookMetadata(u.id(),bookId,body.title(),body.category(),body.synopsis(),body.serialStatus(),body.cover())));}
    ...
    @PutMapping("/books/{bookId}/volumes/{volumeId}/order") ApiResponse<List<Volume>> reorderVolume(HttpServletRequest request,@PathVariable long bookId,@PathVariable long volumeId,@Valid @RequestBody VolumeOrderRequest body){CurrentUser u=current(request);u.require(Role.AUTHOR);return ApiResponse.ok(store.reorderVolume(u.id(),bookId,volumeId,body.orderNo()));}
    ...
    @GetMapping("/books/{bookId}/chapter-candidates") ApiResponse<List<ChapterCandidate>> chapterCandidates(HttpServletRequest request,@PathVariable long bookId){CurrentUser u=current(request);u.require(Role.AUTHOR);return ApiResponse.ok(store.authorChapterCandidates(u.id(),bookId));}
