...
public class AccountController implements UserResolver {
    ...
    public AccountController(NovelStore store){this.store=store;}
    @GetMapping("/profile") ApiResponse<Map<String,Object>> profile(HttpServletRequest request) { CurrentUser u=current(request); return ApiResponse.ok(Map.of("id",u.id(),"name",u.name(),"roles",u.roles())); }
    @GetMapping("/bookshelf") ApiResponse<List<Book>> shelf(HttpServletRequest request) { CurrentUser u=current(request); return ApiResponse.ok(store.shelf(u.id()).stream().map(store::book).toList()); }
    ...
    @PostMapping("/books/{bookId}/reward") ApiResponse<Map<String,Object>> reward(HttpServletRequest request,@PathVariable long bookId,@Valid @RequestBody AmountRequest body) { return ApiResponse.ok(store.reward(current(request).id(),bookId,body.amount())); }
    @PostMapping("/books/{bookId}/purchase") ApiResponse<Map<String,Object>> purchase(HttpServletRequest request,@PathVariable long bookId,@Valid @RequestBody AmountRequest body) { return ApiResponse.ok(store.purchase(current(request).id(),bookId,body.amount())); }
    ...
    public record CommentRequest(Long chapterId,@NotBlank String content) {}
    public record RatingRequest(int rating) {}
    public record AmountRequest(int amount) {}
    public record AuthorApplicationRequest(@NotBlank String penName,@NotBlank String statement) {}
}
