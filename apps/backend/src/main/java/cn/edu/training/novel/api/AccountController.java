...
public class AccountController implements UserResolver {
    ...
    @PostMapping("/books/{bookId}/comments") ApiResponse<Comment> comment(HttpServletRequest request,@PathVariable long bookId,@Valid @RequestBody CommentRequest body) { return ApiResponse.ok(store.comment(current(request),bookId,body.chapterId(),body.content())); }
