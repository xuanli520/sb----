...
| SMTP 邮箱认证 | V27 一次性验证码 HMAC/过期/已用状态、真实 `JavaMailSender` SMTP、发送/验证次数限制、BFF 邮箱注册门禁；V30 支持由唯一站长 `ADMIN` 保存的 AES-GCM 加密 SMTP 设置及测试投递。密码、HMAC 与加密密钥从不出现在 API 响应或日志中；配置不完整或发送失败返回 `503`，不提供模拟或本地回退。 |
...
| 手机登录、微信/QQ OAuth | 产品明确不实现手机号登录，也未提供 OAuth 凭据、回调域名或账号绑定流程；这些入口不属于本期交付。邮箱验证码不在此排除项，已通过真实 SMTP 实现并失败关闭。 |
